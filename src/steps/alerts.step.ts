...
const edgeCasePromptText = "Manual QA 123 !@#$%^&*() - " + 'X'.repeat(80);
...
When('the user dismisses the confirm alert', async function () {
  await PageFactory.alerts(this.page).dismissConfirmAlert();
});
...
When('the user submits edge-case text in the prompt alert', async function () {
  await PageFactory.alerts(this.page).submitPromptAlert(edgeCasePromptText);
});
...
Then('the confirm alert cancel result is displayed', async function () {
  await PageFactory.alerts(this.page).expectConfirmDismissed();
});

Then('the prompt alert displays the edge-case text', async function () {
  await PageFactory.alerts(this.page).expectPromptResult(edgeCasePromptText);
});
